...
class AtariPreprocessor(Preprocessor):
    ...
    We may also want to max over frames to remove flickering. Some games require this (based on animations and the limited sprite drawing capabilities of the original Atari).

    Parameters
    ----------
    new_size: 2 element tuple
    The size that each image in the state should be scaled to. e.g (84, 84) will make each image in the output have shape (84, 84).
    """
    ...
    def process_state_for_memory(self, state):
        ...
        We don't want to save floating point numbers in the replay memory. We get the same resolution as uint8, but use a quarter to an eigth of the bytes (depending on float32 or float64)

        We recommend using the Python Image Library (PIL) to do the image conversions.
    ...
    def process_state_for_network(self, state):
        ...
        Basically same as process state for memory, but this time outputs float32 images.
        """
    ...
    def process_batch(self, states, new_states):
        ...
        Same as process_state_for_network but works on a batch of samples from the replay memory. Meaning we need to convert both state and next state values.
        """
...
class PreprocessorSequence(Preprocessor):
    ...
    For example, if you call the process_state_for_network and you have a sequence of AtariPreproccessor followed by HistoryPreprocessor. 
    This this class could implement a process_state_for_network that does something like the following:

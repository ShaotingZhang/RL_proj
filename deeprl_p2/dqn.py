...
class DQNAgent:
	...
	def update_predict_network(self):
		...
		states, actions, rewards, new_states, is_terminals = self.memory.sample(self.batch_size)
                ...
                actions = self.reprocessor.process_action(actions)
                # update network
                q_values = self.cal_target_q_values(preprocessed_new_states)
                max_q_values = np.max(q_values, axis=1)
                max_q_values[is_terminals] = 0.0
                target = rewords + self.gamma * max_q_values
                target = np.expand_dims(targets, axis=1)

                self.q_network.train_on_batch([prepeocessed_states, actions], targets)
                if self.num_step % self.target)update_freq ==0:

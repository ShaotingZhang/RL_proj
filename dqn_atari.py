...
def create_model(window, input_shape, num_actions, model_name='q_network'):
    ...
    # clear the model
    network_model = None
    # check which network is chosen
    if model_name == 'q_network':
        # build a CNN
        with tf.name_scope('q_network'):
            # prepare the input state and input action
            ...
            # the output layer to get q_values
            ...
            # the first model with input state and input action
            network_model = Model(inputs=[input_state, input_action], outputs=q_v)
            #the second model with input state and q values
...
def get_output_folder(parent_dir, env_name):
    ...
    This is just convenient so that if you run the same script multiple times tensorboard can plot all of the results on the same plots with different names.
